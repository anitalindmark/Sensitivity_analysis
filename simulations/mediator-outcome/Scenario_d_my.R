...
nie.bc.my.500 <- nde.bc.my.500 <- se.nie.bc.my.500 <- se.nde.bc.my.500 <- matrix(nrow = S, ncol = length(rho)) 
states.bc.my.500 <- list() # List to store the states of the random number generator, to be able to replicate results
...
set.seed(40459) # n = 500
# set.seed(42461) # n = 1000
# set.seed(44483) # n = 5000
...
for(i in 1:S){
  ...
  states.bc.my.500[[i]] <- .Random.seed
  ...
  M.star <- med.coefs[1, ] + med.coefs[2, ]*Z + med.coefs[3, ]*X + med.coefs[4, ]*Z*X + epsilon[, 1]
  ...
  result.bc.my.500 <- sensmediation(med.model = m.model, out.model = y.model, Rho = rho,
                                        progress = FALSE, exp.name = "Z", med.name = "M")
  
  
  ###Storage of results:
  nie.bc.my.500[i, ] <- result.bc.my.500$NIE
  nde.bc.my.500[i, ] <- result.bc.my.500$NDE
  se.nie.bc.my.500[i, ] <- result.bc.my.500$std.errs$se.nie
  se.nde.bc.my.500[i, ] <- result.bc.my.500$std.errs$se.nde

  
  if(i==S)
    states.bc.my.500[[i + 1]] <- .Random.seed
